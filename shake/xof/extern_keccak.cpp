...
int keccakInit(KeccakContext *context, uint_t capacity)
{
   uint_t rate;
...
void keccakAbsorb(KeccakContext *context, const void *input, int length)
{
   uint_t i;
   int n;
   keccak_lane_t *a;
   ...
   while(length > 0)
   {
	  ...
	  if( length < (int)(context->blockSize - context->length) )
	  {
		  n = length;
	  }
	  else
	  {
		  n = context->blockSize - context->length;
	  }
...
void keccakSqueeze(KeccakContext *context, uint8_t *output, int length)
{
   uint_t i;
   int n;
   keccak_lane_t *a;
   ...
   while(length > 0)
   {
      ...
      if(output != NULL)
      {
         memcpy(output, context->digest + context->blockSize -
            context->length, n);
      }
